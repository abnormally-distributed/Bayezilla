...
Spike <- function(formula, data, family = "gaussian", log_lik = FALSE, iter = 10000, warmup = 1000, adapt = 2000, chains = 4, thin = 1, method = "parallel", cl = makeCluster(2), ...) {
  ...
  if (family == "gaussian") {
    ...
    inits <- lapply(1:chains, function(z) list("Intercept" = lmSolve(formula, data)[1], .RNG.name = "lecuyer::RngStream", .RNG.seed = sample(1:10000, 1), "ySim" = y, "delta" = rep(1, P),  "phi" = rbeta(1, 2, 2),  "theta" = lmSolve(formula, data)[-1], "tau" = 1))
  ...
  if (family == "binomial" || family == "logistic") {
    ...
    inits <- lapply(1:chains, function(z) list("Intercept" = as.vector(coef(glm(formula, data, family = "binomial")))[1], .RNG.name = "lecuyer::RngStream", .RNG.seed = sample(1:10000, 1), "ySim" = y, "delta" = rep(1, P),  "phi" = rbeta(1, 2, 2),  "theta" = as.vector(coef(glm(formula, data, family = "binomial")))[-1]))
    out <- run.jags(model = "jags_glm_spike.txt", modules = c("glm on", "bugs on", "dic off"), monitor = monitor, data = jagsdata, inits = inits, burnin = warmup, sample = iter, thin = thin, adapt = adapt, method = method, cl = cl, summarise = FALSE,...)
    return(out)
  }
  ...
  if (family == "poisson") {
    ...
    inits <- lapply(1:chains, function(z) list("Intercept" = as.vector(coef(glm(formula, data, family = "poisson")))[1], .RNG.name = "lecuyer::RngStream", .RNG.seed = sample(1:10000, 1), "ySim" = y, "delta" = rep(1, P), "phi" = rbeta(1, 2, 2),  "theta" = as.vector(coef(glm(formula, data, family = "poisson")))[-1]))
    out <- run.jags(model = "jags_glm_spike.txt", modules = c("glm on", "bugs on", "dic off"), monitor = monitor, data = jagsdata, inits = inits, burnin = warmup, sample = iter, thin = thin, adapt = adapt, method = method, cl = cl, summarise = FALSE, ...)
