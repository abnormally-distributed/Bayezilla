...
#' SASP()
#'
bridgeGlm = function(formula, data, family = "gaussian", kappa = .5, log_lik = FALSE, iter= 4000, warmup = 1000, adapt=5000, chains=4, thin = 1, method = "parallel", cl = makeCluster(3), ...){
  ...
  Deviance <- -2 * sum(log_lik[1:N])
    ...
    P = ncol(X)
    monitor = c("Intercept", "beta", "sigma", "Deviance", "lambda", "ySim" ,"log_lik")
    if (log_lik == FALSE){
      monitor = monitor[-(length(monitor))]
    }
    jagsdata = list(X = X, y = y, N = length(y), P = ncol(X), "kappa" = kappa)
    inits = lapply(1:chains, function(z) list("Intercept" = 0, "ySim" = y, "xi" = rep(1, P), "u" = rep(1, P), "eta" = rep(1, P), "lambda" = 10, "tau" = 1 , .RNG.name="lecuyer::RngStream", .RNG.seed= sample(1:10000, 1)))  
