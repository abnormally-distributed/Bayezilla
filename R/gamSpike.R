#' Bayesian GAMs with Variable Selection
#'
...
#'
#' 
gamSpike  = function(formula, data, family = "gaussian", log_lik = FALSE, iter= 4000, warmup=1000, adapt=1000, chains=4, thin=1, method = "rjparallel", cl = makeCluster(2), ...){
  ...
  if (family == "gaussian"){
    ...
    jags_glm = "model{
              ...
              for (p in 1:P){
                theta[p] ~ dnorm(0, eta[idx[p]])
                beta[p] <- delta[p] * theta[idx[p]]
              }
    ...
    monitor = c("Intercept", "beta", "sigma", "eta", "delta", "phi", "lambda", "fX", "Deviance", "ySim" ,"log_lik")
    if (log_lik == FALSE){
    ...
    inits = lapply(1:chains, function(z) list("Intercept" = lmSolve(formula, data)[1], 
                                              ...
                                              "lambda" = 2,
                                              "phi" = rbeta(1, 4, 4),  
                                              "delta" = sample(c(0, 1), replace = TRUE, size = max(idx)),
  ...
  if (family == "binomial"){
    ...
    write_lines(jags_glm, "jags_glm.txt")
    jagsdata = list(X = X, y = y,  N = length(y), P = ncol(X), S = max(idx), idx = idx, start = start, end = end)
    monitor = c("Intercept", "beta", "sigma", "eta", "lambda", "delta", "phi", "fX", "Deviance", "ySim" ,"log_lik")
    if (log_lik == FALSE){
      monitor = monitor[-(length(monitor))]
    }
