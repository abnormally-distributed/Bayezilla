...
#' and poisson likelihoods the same pseudovariance functions used in all other models in this package are used. \cr
#' \cr
...
gdp = function(formula, data, log_lik = FALSE, iter=10000, warmup=1000, adapt=2000, chains=4, thin=1, method = "parallel", cl = makeCluster(2), ...){
  ...
  monitor <- c("Intercept", "beta", "sigma", "Deviance", "alpha", "zeta", "lambda", "ySim", "log_lik")
  if (log_lik == FALSE){
  ...
  Deviance <- -2 * sum(log_lik[1:N])
    ...
    monitor <- c("Intercept", "beta", "Deviance", "alpha", "zeta", "lambda", "ySim", "log_lik")
    if (log_lik == FALSE){
      monitor = monitor[-(length(monitor))]
    }
