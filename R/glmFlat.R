...
#' have normal(0, 1e-200) priors (precision of 1e-200 is equivalent to a standard 
#' deviation of 1e100) and the precision has a gamma(1e-200, 1e-200) prior for
#' the Gaussian likelihood. No data that 
#' would ever be observed would be anywhere near this scale, so this can be used 
...
#' The results from this should be equivalen to using lm() or glm()
#' in R.
#' 
#' 
...
#' 
glmFlat  = function(formula, data, family = "gaussian", log_lik = FALSE, iter=10000, warmup=2500, adapt=2500, chains=4, thin=2, method = "parallel", cl = makeCluster(2), ...){
  ...
  if (family == "gaussian"){
    ...
    inits = lapply(1:chains, function(z) list("Intercept" = 0, "beta" = jitter(rep(0, P), amount = 1), "tau" = 1, "ySim" = y, .RNG.name= "lecuyer::RngStream", .RNG.seed = sample(1:20000, 1)))
  }
  ...
  if (family == "binomial" || family == "logistic"){
    ...
    inits = lapply(1:chains, function(z) list("Intercept" = 0, "beta" = jitter(rep(0, P), amount = 1), "ySim" = y, .RNG.name= "lecuyer::RngStream", .RNG.seed= sample(1:20000, 1)))
  }
  ...
  if (family == "poisson"){
    ...
    inits = lapply(1:chains, function(z) list("Intercept" = 0, "ySim" = y, .RNG.name= "lecuyer::RngStream", .RNG.seed= sample(1:20000, 1),"beta" = jitter(rep(0, P), amount = 1)))
  }
