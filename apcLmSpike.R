apcLmSpike = function(formula, data, phi_prior = .5 , lambda = -1, log_lik = FALSE, iter=10000, warmup=1000, adapt=5000, chains=4, thin=3,method = "rjags", cl = NULL, ...)
{
  ...
  jagsdata = list(X = X, y = y, phi = phi_prior, N = length(y), P = ncol(X), prior_cov = prior_cov, zeros = rep(0, P))
  monitor = c("Intercept", "beta", "sigma", "g", "deviance", "delta", "ySim", "log_lik")
  if (log_lik == FALSE){
    monitor = monitor[-(length(monitor))]
  }
  inits = lapply(1:chains, function(z) list("Intercept" = 0, "delta" = rep(1, P), "theta" = jitter(rep(0, P), amount = .25), "g_inv" = .001, "tau" = 3, "ySim" = y))
