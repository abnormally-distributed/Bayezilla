...
P <- 15
#True betas
betas = c(.2927551, .271, 2.153434, -.51, 0, 1.08113776, -0.89273929, 0.171721614, 0, -.25, -1.18113776, -0.9273929, 0.72321414, 0.13034, 1.1) 
...
Sigma = jitter(diag(rep(1, P)), amount = .85)
neg = which(Sigma < -1)
pos = which(Sigma > 1)
Sigma[neg] <- -1
Sigma[pos] <- 1
...
X = X + matrix(rgamma((N * P)/4 , .5, 10), N, P)
...
```{r}
post_summary(out2)
coefs2 = post_summary(out2)$estimate[2:16]
```
...
```{r}
post_summary(out3)
coefs3 = post_summary(out3)$estimate[2:16]
```
...
```{r}
post_summary(out4)
coefs4 = post_summary(out4)$estimate[2:16]
```
...
```{r}
post_summary(out5)
coefs5 = post_summary(out5)$estimate[2:16]
```
...
```{r}
true.betas = round(c(.2927551, .271, 2.153434, -.51, 0, 1.08113776, -0.89273929, 0.171721614, 0, -.25, -1.18113776, -0.9273929, 0.72321414, 0.13034, 1.1) , 3)
