...
<p><br />
<span class="math-tex"> \(\mathcal{C}_{\mathcal{i}} = \frac{1}{∑_{j\neq i}^N d(n_i,e_{ij})}\)</span><br />
&nbsp;</p>
...
<p><span class="math-tex">\(C_i^w=\frac{\displaystyle\sum\nolimits_{j,q} (w_{(j,i)}w_{(i,q)}w_{(j,q)}) } {{{\displaystyle\sum\nolimits_{q}w_{(i,q)}}}^2-{\displaystyle\sum\nolimits_{q}{w^2_{(i,q)}}}}\)</span></p>
...
<p><span class="math-tex">\(C_{i}^{sw}=\frac{\sum_{j, q}\left(w_{s(j, i)} w_{s(i, q)} w_{s(j, q)}\right)}{\sum_{j \neq q}\left|w_{s(j, i)} w_{s(i, q)}\right|}\)</span></p>
...
<p><br />
<span class="math-tex">
\(E_{L}(G)= \sum_{i=1}^nd_i^2+2\sum_{i,j} w_{i\text{,}j}^2\)
</span><br />
&nbsp;</p>
...
Another method more recently added to the package is the iterative Louvain algorithm for signed networks. This has been adapted from the Brain Connectivity Toolbox in Matlab (Rubinov & Sporns, 2011) into R code. The algorithm is a stochastic resampling based algorithm that applies the classical deterministic Louvain algorithm to resampled networks, and calculates module assignments. Label switching over different iterations is accounted for. <br>
